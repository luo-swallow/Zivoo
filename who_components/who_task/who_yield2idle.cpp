...
// Default WDT timeout if Task WDT is not enabled in sdkconfig
...
void WhoYield2Idle::task()
{
    ...
    if (interval < pdMS_TO_TICKS(1500)) {
        ESP_LOGW(TAG, "Try to increase CONFIG_ESP_TASK_WDT_TIMEOUT_S (current: %d s)", CONFIG_ESP_TASK_WDT_TIMEOUT_S);
    }
