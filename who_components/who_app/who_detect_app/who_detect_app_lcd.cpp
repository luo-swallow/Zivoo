...
WhoDetectAppLCD::WhoDetectAppLCD(const std::vector<std::vector<uint8_t>> &palette,
    ...
    WhoDetectAppBase(frame_cap), m_parent(parent)
{
    if (!lcd_disp_frame_cap_node) {
        lcd_disp_frame_cap_node = frame_cap->get_last_node();
    }
    m_lcd_disp = new lcd_disp::WhoFrameLCDDisp("LCDDisp", lcd_disp_frame_cap_node, -1, m_parent);
    WhoApp::add_task(m_lcd_disp);
    m_lcd_disp->set_lcd_disp_cb(std::bind(&WhoDetectAppLCD::lcd_disp_cb, this, std::placeholders::_1));
#if !BSP_CONFIG_NO_GRAPHIC_LIB
    m_result_lcd_disp = new lcd_disp::WhoDetectResultLCDDisp(m_detect, m_lcd_disp->get_canvas(), palette, class_names, font);
#else
    m_result_lcd_disp = new lcd_disp::WhoDetectResultLCDDisp(m_detect, nullptr, palette);
#endif
