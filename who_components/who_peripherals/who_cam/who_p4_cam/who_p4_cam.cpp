...
{
    // 检查是否为 RAW10 格式（不支持，需跳过）
#if defined(CONFIG_CAMERA_SC2336_MIPI_IF_FORMAT_INDEX_DEFAULT)
    if (CONFIG_CAMERA_SC2336_MIPI_IF_FORMAT_INDEX_DEFAULT < 8) {
        ESP_LOGE(TAG, "raw10 cam mode is not supported.");
        close(m_fd);
        return ESP_FAIL;
    }
#elif defined(CONFIG_CAMERA_OV5647_MIPI_IF_FORMAT_INDEX_DEFAULT)
