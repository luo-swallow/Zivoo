...
WhoFrameLCDDisp::WhoFrameLCDDisp(const std::string &name, frame_cap::WhoFrameCapNode *frame_cap_node, int peek_index, lv_obj_t *parent) :
    ...
    m_parent(parent),
#endif
    m_frame_cap_node(frame_cap_node), m_peek_index(peek_index)
{
    frame_cap_node->add_new_frame_signal_subscriber(this);
#if !BSP_CONFIG_NO_GRAPHIC_LIB
    bsp_display_lock(0);
    lv_obj_t *parent_obj = m_parent ? m_parent : lv_scr_act();
    m_canvas = lv_canvas_create(parent_obj);
...
lv_obj_t *WhoFrameLCDDisp::get_parent()
{
    return m_parent;
}
#endif
